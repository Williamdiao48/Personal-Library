...
export default function CollectionView() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([cols, its, tags, itemTags, itemCols, trashed, allItems]) => {
        ...
        if (its.length > 0)
          collectionService.reorderItems(
            id,
            its.map((i) => i.id),
          )
      })
  ...
  const handleCollectionRename = useCallback(
    async (colId: string, name: string) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (typeFilters.length > 0) result = result.filter((i) => typeFilters.includes(i.content_type))

    if (tagFilters.length > 0)
      result = result.filter((i) =>
        (itemTagsMap[i.id] ?? []).some((t) => tagFilters.includes(t.id)),
      )
    ...
    <div className="library-layout">
      ...
      <main className="library-main" ref={mainRef}>
        ...
        <div className="library-filter-bar">
          ...
          {dragEnabled && <span className="collection-drag-hint">Drag cards to reorder</span>}
          {sortBy === 'custom' && hasActiveFilters && (
            <span className="collection-drag-hint collection-drag-hint--muted">
        ...
        {loading ? (
          ...
          <div className="library-state-center">
            <p className="state-text">Failed to load collection: {loadError}</p>
          </div>
        ) : items.length === 0 ? (
          <div className="library-state-center">
          ...
          <div className="library-state-center">
            <p className="state-text">No items match your filters.</p>
          </div>
        ) : (
          <DndContext
          ...
          >
            <SortableContext items={displayedItems.map((i) => i.id)} strategy={rectSortingStrategy}>
              <div
                ...
                style={{ gridTemplateColumns: `repeat(${columnsPerRow}, ${colWidth}px)` }}
              >
