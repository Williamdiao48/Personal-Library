...
export default function LibraryView() {
  ...
  const groupedEpubIds = useMemo(() => {
    ...
    return new Set(
      items.filter((i) => i.derived_from && sourceIds.has(i.derived_from)).map((i) => i.id),
    )
  }, [items])
  ...
  const universalTagIds = useMemo(() => {
    ...
    for (const eid of editableIds)
      for (const t of itemTagsMap[eid] ?? []) counts.set(t.id, (counts.get(t.id) ?? 0) + 1)
    return new Set(
      [...counts.entries()].filter(([, n]) => n === editableIds.length).map(([id]) => id),
    )
  ...
  const universalColIds = useMemo(() => {
    ...
    const counts = new Map<string, number>()
    for (const eid of editableIds)
      for (const c of itemCollectionsMap[eid] ?? []) counts.set(c.id, (counts.get(c.id) ?? 0) + 1)
    return new Set(
      [...counts.entries()].filter(([, n]) => n === editableIds.length).map(([id]) => id),
    )
              ...
              return (
                ...
                >
                  {rowItems.map((item) => {
                    ...
                    const sourceItem =
                      companion && pref === 'pdf' ? companion : companion ? item : undefined
                    ...
                    return (
                      <div key={item.id} style={{ width: `${colWidth}px`, flexShrink: 0 }}>
                        <ItemCard
                          ...
                          onClick={(e) =>
                            handleCardClick(item.id, e, () => navigate(`/read/${displayItem.id}`))
                          }
                          onOpenSource={
                            sourceItem ? () => navigate(`/read/${sourceItem.id}`) : undefined
                          }
                          onTogglePreferred={
                            companion ? () => handleTogglePreferred(item.id) : undefined
                          }
                          onDelete={async () => {
                          ...
                          onAuthorChange={(author) =>
                            setItems((prev) =>
                              prev.map((i) => (i.id === displayItem.id ? { ...i, author } : i)),
                            )
                          }
                          onTitleChange={(title) =>
                            setItems((prev) =>
                              prev.map((i) => (i.id === displayItem.id ? { ...i, title } : i)),
                            )
                          }
                          ...
                          onRatingChange={(rating) => {
                            ...
                            setItems((prev) =>
                              prev.map((i) => (i.id === editableItem.id ? { ...i, rating } : i)),
                            )
                          }}
                          ...
                          onAppend={
                            displayItem.source_url && displayItem.chapter_end != null
      ...
      {selectedIds.size > 0 && (
        <div className="bulk-action-bar" onClick={(e) => e.stopPropagation()}>
          <span className="bulk-action-count">
            ...
            {selectedIds.size === 1 && (
              <span className="bulk-action-hint"> · Shift+click to add more</span>
            )}
          </span>
                  ...
                  allTags.map((t) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          applied ? handleBulkRemoveTag(t.id) : handleBulkAddTag(t.id)
                        }
                      >
                        <span
                          style={{
                  ...
                  allCollections.map((c) => {
                    ...
                    return (
                      <button
                        ...
                        className={`bulk-popover-item${applied ? ' bulk-popover-item--applied' : ''}`}
                        onClick={() =>
                          applied ? handleBulkRemoveCollection(c.id) : handleBulkAddCollection(c.id)
                        }
                      >
                        {c.name}
                        {applied && (
          ...
          {bulkDeleteConfirming ? (
            <>
              <span className="bulk-action-confirm-text">
      ...
      {reviewModalItem && (
        <ReviewModal
          ...
          onSave={(review, rating) => {
            setItems((prev) =>
              prev.map((i) => (i.id === reviewModalItem.id ? { ...i, review, rating } : i)),
            )
