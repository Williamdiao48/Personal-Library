...
export default function AddToCollectionModal({
  collectionId, collectionName, existingItemIds, onAdd, onClose,
}: Props) {
  ...
  async function handleAdd(item: Item) {
    ...
    try {
      ...
      setAdding(prev => { const next = new Set(prev); next.delete(item.id); return next })
    }
  }
    ...
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="modal atc-modal">
        <div className="atc-modal-header">
          ...
          <button className="atc-modal-close" onClick={onClose} aria-label="Close">✕</button>
        </div>
        ...
        <div className="library-search atc-search">
          ...
          />
          {query && (
            <button className="library-search-clear" onClick={() => setQuery('')} aria-label="Clear">✕</button>
          )}
        </div>
          ...
          {loading ? (
            ...
            filteredItems.map(item => (
              <div key={item.id} className="atc-row">
                ...
                <span className="atc-row-type">{TYPE_LABELS[item.content_type] ?? item.content_type}</span>
                <button
