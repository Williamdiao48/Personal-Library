...
export const goalsService = {
  ...
  create: (payload: {
    ...
    targetCount?: number
  }): Promise<Goal> => window.api.goals.create(payload),
  update: (
    id: string,
    patch: {
      title?: string
  ...
  ): Promise<void> => window.api.goals.update(id, patch),
  delete: (id: string): Promise<void> => window.api.goals.delete(id),
  addItem: (goalId: string, itemId: string): Promise<void> =>
    window.api.goals.addItem(goalId, itemId),
  removeItem: (goalId: string, itemId: string): Promise<void> =>
    window.api.goals.removeItem(goalId, itemId),
  upsertPeriodGoal: (
    type: 'time' | 'count',
    period: GoalPeriod,
    target: number | null,
  ): Promise<Goal | null> => window.api.goals.upsertPeriodGoal(type, period, target),
}
