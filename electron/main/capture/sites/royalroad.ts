...
export async function captureRoyalRoad(
  ...
  const title =
    ...
    'Unknown Story'
  const author =
    doc.querySelector('.fic-header [href*="/profile/"]')?.textContent?.trim() ??
    doc.querySelector('meta[name="author"]')?.getAttribute('content')?.trim() ??
    null
  const coverUrl =
    doc.querySelector('.thumbnail, .fic-image img')?.getAttribute('src') ??
  ...
  for (const a of Array.from(
    doc.querySelectorAll<HTMLAnchorElement>('table#chapters tbody tr td a[href*="/chapter/"]'),
  )) {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const chTitle =
      cdoc.querySelector('.chapter-title, h1')?.textContent?.trim() ?? `Chapter ${i + 1}`
