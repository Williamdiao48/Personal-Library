...
export async function captureXenForo(
  ...
  const title =
    firstPageDoc.querySelector('h1.p-title-value')?.textContent?.trim() ??
    firstPageDoc.querySelector('h1')?.textContent?.trim() ??
    'Unknown Story'
  const author = firstPageDoc.querySelector('.username')?.textContent?.trim() ?? null
  const coverUrl =
    firstPageDoc.querySelector('meta[property="og:image"]')?.getAttribute('content') ?? null
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const postEl = postId
      ? (pdoc.getElementById(`post-${postId}`) ??
