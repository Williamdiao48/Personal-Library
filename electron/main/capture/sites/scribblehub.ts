...
export async function getScribbleHubChapterCount(url: string): Promise<number | null> {
  ...
  try {
    ...
    const count = new JSDOM(html).window.document.querySelectorAll(
      'li.toc_w a[href*="/read/"]',
    ).length
...
export async function captureScribbleHub(
  ...
  const title =
    seriesDoc.querySelector('.fic-title')?.textContent?.trim() ??
    seriesDoc.querySelector('h1')?.textContent?.trim() ??
    'Unknown Story'
  const author =
    seriesDoc.querySelector('.auth_name_fic')?.textContent?.trim() ??
    seriesDoc.querySelector('meta[name="author"]')?.getAttribute('content')?.trim() ??
    null
  const coverUrl =
    seriesDoc.querySelector('.fic-image img')?.getAttribute('src') ??
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const chTitle =
      cdoc.querySelector('.chapter-inner-header h1, .wi_fic_title, h1')?.textContent?.trim() ??
      `Chapter ${i + 1}`
    ...
    chapters.push({
      title: chTitle,
      html: sanitize(content.innerHTML),
