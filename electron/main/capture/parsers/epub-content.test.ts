...
describe('transformChapterHtml (F9 DOM rewriting)', () => {
  ...
  it('inlines a relative <img> as a base64 data URI', () => {
    const out = transformChapterHtml('<img src="images/p.png">', ctx({
      xhtmlDir: 'text/',
  ...
  it('rewrites a cross-chapter link to data-epub-chapter + fragment, dropping href', () => {
    const out = transformChapterHtml('<a href="ch2.xhtml#sec">next</a>', ctx({
      xhtmlDir: 'text/',
  ...
  it('strips a spoofed pre-existing data-epub-chapter and uses the resolved index', () => {
    const out = transformChapterHtml('<a href="ch2.xhtml" data-epub-chapter="999">x</a>', ctx({
      xhtmlDir: 'text/',
  ...
  it('cannot smuggle a live <img>/onerror through a crafted fragment value', () => {
    ...
    const out = transformChapterHtml('<a href="#a&quot;&gt;&lt;img src=x onerror=alert(1)&gt;">x</a>', ctx())
    const d = parse(out)
    expect(d.querySelector('img')).toBeNull() // stays inert inside the attribute value
    const hasOnerror = Array.from(d.querySelectorAll('*')).some(el => el.getAttributeNames().includes('onerror'))
  ...
  it('strips a leading heading that is exactly the book title', () => {
    const out = transformChapterHtml('<h1>My Book</h1><p>Real text.</p>', ctx({ bookTitle: 'My Book' }))
    expect(out).not.toMatch(/my book/i)
    expect(out).toContain('Real text.')
  })

  it('keeps a leading heading that is not the book title', () => {
    const out = transformChapterHtml('<h1>Chapter One</h1><p>Body.</p>', ctx({ bookTitle: 'My Book' }))
    expect(out).toContain('Chapter One')
  })

  it('matches the title through HTML entities (&amp;)', () => {
    const out = transformChapterHtml('<p>Cats &amp; Dogs</p><p>Body.</p>', ctx({ bookTitle: 'Cats & Dogs' }))
